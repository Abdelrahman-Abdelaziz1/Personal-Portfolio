...
export const Vortex = (props: VortexProps) => {
  ...
  const rand = (n: number): number => n * Math.PI * 2 * Math.random();
  ...
  const updateParticle = (i: number, ctx: CanvasRenderingContext2D) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    let x, y, vx, vy, life, ttl, speed, radius, hue, n, theta, force;
    x = particleProps[i];
    y = particleProps[i + 1];
    n = noise3D(x * 0.00125, y * 0.00125, tick * 0.0005) * 1;
    theta = n * TAU;
    vx = lerp(particleProps[i + 2], Math.cos(theta), 0.05);
    vy = lerp(particleProps[i + 3], Math.sin(theta), 0.05);
